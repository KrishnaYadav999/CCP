            ...
            <div
              ...
              className={`group relative min-h-[120px] overflow-hidden rounded-2xl border px-5 py-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl sm:px-6 ${
                index === 0
                  ...
                  : 'border-red-100 bg-white text-slate-900 hover:border-red-200'
              }`}
            >
              <div className={`absolute -right-10 -top-10 h-32 w-32 rounded-full transition duration-300 group-hover:scale-125 ${index === 0 ? 'bg-white/10' : 'bg-red-50'}`} />
              <div className={`absolute bottom-0 left-0 h-1 w-full ${index === 0 ? 'bg-emerald-300/80' : 'bg-red-400/70'}`} />
              <div className="relative flex h-full items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className={`mt-2 text-sm font-bold ${index === 0 ? 'text-emerald-50/80' : 'text-slate-500'}`}>
                    {index === 0 ? 'Ready for assignment' : 'Needs attention'}
                  </p>
                </div>
